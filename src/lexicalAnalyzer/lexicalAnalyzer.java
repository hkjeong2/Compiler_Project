...
import lexicalAnalyzer.DFAmachine.*;
import file.file;
...
public class lexicalAnalyzer {
    ...
    public lexicalAnalyzer(Stack<ArrayList<Integer>> pos, ArrayList<String> tokenName, ArrayList<String> tokenValue){
        this.pos = pos;
        this.tokenName = tokenName;
        this.tokenValue = tokenValue;
        machines.add(new assignmentMachine(tokenName, tokenValue));
        machines.add(new bracesMachine(tokenName, tokenValue));
        ...
        machines.add(new parenthesesMachine(tokenName, tokenValue));
        ...
        for (String s : inputToken) {
            ...
            while (s.substring(idx).length() != 0) {
                ...
                if (mIdx <= 0 || !machines.get(mOrder).getAccepted()[machines.get(mOrder).getCurrentState()]) {
                    ...
                    throw new Exception();
                } else { //오류가 없다면 검수가 완료된 다음 지점부터 분석을 재시작 하기 위해 idx update, token name 및 value와 해당 토큰 위치 저장
                    if (s.charAt(idx)!=' '){
                        pos.push(new ArrayList<>(Arrays.asList(line, idx)));
                    }
                    idx += mIdx;
